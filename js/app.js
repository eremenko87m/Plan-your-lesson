...
async function load(){
  try{applyData(await fetchData(false))}
...
function setRefreshButtons(text,disabled=false){
  for(const id of ['#refreshBtn','#refreshBtnSide']){
    const b=$(id);if(!b)continue;b.textContent=text;b.disabled=disabled;b.style.opacity=disabled?'.72':'1';
  }
}
async function refreshView(){
  setRefreshButtons('Refreshing…',true);
  try{
    const before=S.data?.sourceFingerprint||S.data?.updatedAt||'';
    const fresh=await fetchData(true);
    applyData(fresh);
    const after=fresh?.sourceFingerprint||fresh?.updatedAt||'';
    setRefreshButtons(after&&after!==before?'Updated ✓':'Refreshed ✓',true);
    setTimeout(()=>setRefreshButtons('Refresh view',false),1700);
  }catch(e){
    console.error(e);
    setRefreshButtons('Refresh failed',true);
    setTimeout(()=>setRefreshButtons('Refresh view',false),2200);
  }
}
function openSyncPage(){
  const repo=repoInfo();
  if(repo){window.open(`https://github.com/${repo.owner}/${repo.repo}/actions/workflows/sync.yml`,'_blank');}
  else{alert('Open your GitHub repository and run: Actions → Sync LessonFlow from Yandex → Run workflow.');}
}
...
$('#refreshBtn').onclick=refreshView;$('#refreshBtnSide').onclick=refreshView;
$('#syncNowBtn').onclick=openSyncPage;$('#syncNowBtnSide').onclick=openSyncPage;
